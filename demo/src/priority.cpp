...
void Sample::Priority(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
     ...
     * The circles segment will be used to demonstrate the use of priority on geometry residing in different segments
     * The shapes segment will be used to demonstrate the use of priority on geometry sharing the same segment
     *
     * In order for priority to take effect
     * set the rendering algorithm to priority */
    view.Update();
    modelKey.GetVisibilityControl().SetEdges(true);
    modelKey.GetTextAttributeControl().SetAlignment(Text::Alignment::CenterRight);
    ...
    circles.GetSubwindowControl().SetRenderingAlgorithm(Subwindow::RenderingAlgorithm::Priority);
    shapes.GetSubwindowControl().SetRenderingAlgorithm(Subwindow::RenderingAlgorithm::Priority);
    ...
    CircleKey c2 = circle2.InsertCircle(Point(0.5f, 0, 0), 1.0f, Vector(0, 0, 1));
    ...
    points.push_back(Point(1, -0.5f, 0));
    points.push_back(Point(0, -0.5f, 0));
    CircleKey shape1 = shapes.InsertCircle(Point(0, 0, 0), 1.0f, Vector(0, 0, 1));
    PolygonKey shape2 = shapes.InsertPolygon(points);
    CircleKey shape3 = shapes.InsertCircle(Point(1, 0, 0), 1.0f, Vector(0, 0, 1));
