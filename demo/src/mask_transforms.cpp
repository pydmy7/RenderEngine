...
void Sample::Mask_Transforms(WindowKey wk, Canvas canvas, View view,
    ...
    modelKey.GetMaterialMappingControl()
        .SetFaceColor(RGBAColor(0, 1, 0, 1))
        .SetEdgeColor(RGBAColor(0, 0, 1, 1));
    modelKey.GetTextAttributeControl()
        .SetSize(0.02f, Text::SizeUnits::WindowRelative)
     ...
     *
     * Each segment contains a cube, which is included from the root segment
     * previously created All geometry is set to have green faces. This
     * attribute is overridden in r_masked where face color is changes to red.
     *
     * Modelling matrix rotation is masked in r_masked by using a
     ...
     * Each segment contains a cube, which is included from the root segment
     * previously created All geometry is set to have green faces. This
     * attribute is overridden in t_masked where face color is changes to red.
     *
     * Modelling matrix translation is masked in t_masked by using a
     * ModellingMatrixControl */
    ...
    maskTranslationSegment.InsertText(Point(0, 1.85f, 0),
                                      "Masking Translation");
     ...
     * Each segment contains a cube, which is included from the root segment
     * previously created All geometry is set to have green faces. This
     * attribute is overridden in s_masked where face color is changes to red.
     *
     * Modelling matrix translation is masked in s_masked by using a
     * ModellingMatrixControl */
    ...
    /* This next block of code rotates the maskRotationSegment. We expect that
     * the green cube in that segment will rotate, while the red cube in its
     * r_masked subsegment will stay still since rotation is masked.
     *
     * The same will happen for the segments masking scaling and rotation,
     * relative to those transformations*/
    for (int i = 0; i < 360; i++) {
        maskRotationSegment.GetModellingMatrixControl().Rotate(0, 1, 0);
        maskScalingSegment.GetModellingMatrixControl().Scale(1.001f, 1.001f,
                                                             1.001f);
        maskTranslationSegment.GetModellingMatrixControl().Translate(0, 0.001f,
                                                                     0);
