...
void Sample::Meshes(WindowKey wk, Canvas canvas, View view, Model model,
                    SegmentKey modelKey) {
    /* Sample: Meshes
     ...
     *window
     * 2. The CreateToroid method inserts a toroid in the segment whose key is
     *passed as the first argument
     * 3. Please see the body of CreateToroid for details on how to insert a
     *mesh and set its normals */
    ...
    /* This block of code creates three segments below modelKey and includes
     *geometry in them
     * 1. To create a child segment use the following method:
     *		SegmentKey childSegmentKey=
     *parentSegmentKey.Subsegment("optional_child_segment_name")
     ...
     *	3. Segment level attributes can be set either with kits or controls.
     *Controls interact directly with the database, while kits exist outside of
     *it, and only change the database when they are applied on a segment.
     *Attributes not explicitly set on a segment are inherited from parent
     *segments. Attributes set at a shell level have precedence over those set
    ...
    greenRingKey.GetMaterialMappingControl().SetFaceColor(
        RGBAColor(0, 1, 0, 1));
