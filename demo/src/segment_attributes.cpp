...
void Sample::Segment_Attributes(WindowKey wk, Canvas canvas, View view,
     ...
     * These attributes will be inherited by the subsegments of modelKey, unless
     * overwritten To set colors on different types of geometry, use the
     * MaterialMapping control*/
     ...
     * Each segment gets its own sub-window, all segments are direct children of
     *the modelKey segment
     ...
     * 3. We expect the attributes of each sub-window to be the same of those of
     *the parents segments, unless explicitly modified. There is no need to
     *explicitly specify an attribute if it is getting inherited from a parent
     *segment
     * 4. To add transparency to the contents of a segment, change the alpha
     *value of the RGBAColor passed to one of the methods responsible for
     *changing the color of geometry*/
    ...
    transparentBox.GetMaterialMappingControl().SetFaceColor(
        RGBAColor(1, 0, 0, 0.5f));
