...
void Sample::Image_Output(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
    ...
    try {
        ...
        streamNotifier = Stream::File::Import(static_cast<char const*>(filename), importOptionsKit);
        streamNotifier.Wait();
    }
    catch (IOException e) {
    ...
    /* This block of code shows how to create an off-screen window and how to render to it
     * 1. Create an OffScreenWindowOptionsKit object, and set all the options related to the off-screen window
     *		such as the driver to use, or whether it will use anti-aliasing
     * 2. To have an off-screen window with the same dimensions of the current on-screen window the size in pixels
     *		of the on-screen window can be obtained from the WindowInfoControl
     * 3. Crate an OffscreenWindow from the Database. Pass the width, height and options to the constructor
     * 4. Set a camera, visibility and any other attribute as necessary on the off-screen window. In order to obtain an exact
     *		snapshot of the on-screen window, these attributes should be the same between on- and off-screen windows
     * 5. Include a segment containing geometry into the off-screen window. Only geometry which is either in the included segment
     *		or under it in the scene graph will be rendered to the off-screen window
     * 6. Update the off-screen window to start rendering to it. Since this operation occurs on a separate thread, it is
     *		possible to wait for it to complete by using the UpdateNotifier::Wait() method */
    CameraKit cameraKit;
    ...
    canvas.GetWindowKey().GetWindowInfoControl().ShowPhysicalPixels(width, height);
    ...
    /* This block of code shows how to output the image rendered to the off-screen window to a file
     * 1. Obtain an imageKit from the off-screen window's WindowOptionControl
     * 2. Obtain the image data as an array of bytes using the ImageKit::ShowData() method
    ...
    offscreenWindowKey.GetWindowOptionsControl().ShowImage(Image::Format::Jpeg, imageKit);
