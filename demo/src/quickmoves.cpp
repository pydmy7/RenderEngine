...
void Sample::Quickmoves(WindowKey wk, Canvas canvas, View view, Model model,
                        SegmentKey modelKey) {
    /* Sample: Quickmoves
    ...
    /* This block of code shows how to include a segment into other segments and
     *how to create subwindows
     ...
     *		the values of the arguments in the Rectangle constructors should
     *be between -1 and 1 with the top left corner representing the point (-1,
     *1) */
    ...
    window1.GetTextAttributeControl().SetTransform(
        Text::Transform::NonTransformable);
    ...
    window2.GetTextAttributeControl().SetTransform(
        Text::Transform::NonTransformable);
    ...
    window3.GetTextAttributeControl().SetTransform(
        Text::Transform::NonTransformable);
    ...
    /* This block of code shows how to set different overlays settings on a
     * segments The code will then execute a series of update on a subwindow at
     * the time, allowing you to see the performance difference between segments
     * with the same geometry but difference overlay settings */
    locators[0].GetDrawingAttributeControl().SetOverlay(Drawing::Overlay::None);
    locators[1].GetDrawingAttributeControl().SetOverlay(
        Drawing::Overlay::Default);
    locators[2].GetDrawingAttributeControl().SetOverlay(
        Drawing::Overlay::WithZValues);
