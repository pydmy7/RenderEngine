...
HPS::UTF8 Sample::GetExecutableDirectory() {
    ...
    GetModuleFileNameA(nullptr, executableDirectory,
                       sizeof(executableDirectory));
    char* lastSlash = strrchr(executableDirectory, '\\');
    lastSlash[0] = '\0';
#else
    char executableDirectory[PATH_MAX];
    ssize_t bytes = readlink("/proc/self/exe", executableDirectory,
                             sizeof(executableDirectory));
    char* lastSlash = strrchr(executableDirectory, '/');
    lastSlash[0] = '\0';
#endif
...
bool Sample::ProcessArguments(int argc, char** argv) {
    ...
    while (argCount < argc) {
        ...
        if (currentArg[0] != '-') return false;

        if (currentArg[1] != 't' && currentArg[1] != 'd' &&
            currentArg[1] != 'a' && currentArg[1] != 'c')
            return false;
        else {
            flag = currentArg[1];
            currentArg++;
            if (flag == 't') {
                ...
                std::transform(
                    testNameString.begin(), testNameString.end(),
                    testNameString.begin(), [](unsigned char c) {
                        return static_cast<unsigned char>(::tolower(c));
                    });
                testToRun->testName = UTF8(testNameString.c_str());
                Test item(-1, testToRun->testName);
                auto it = std::find(availableTests.cbegin(),
                                    availableTests.cend(), item);

                if (it == availableTests.cend()) return false;
            ...
            if (flag == 'd') {
                if (argCount < argc - 1) {
                    ...
                    std::transform(
                        driverNameString.begin(), driverNameString.end(),
                        driverNameString.begin(), [](unsigned char c) {
                            return static_cast<unsigned char>(::tolower(c));
                        });
...
void Sample::RunTest(Test* test) {
    ...
    if (driver == "opengl") driverToUse = Window::Driver::OpenGL;
#ifdef _MSC_VER
        ...
        std::cout << "Driver " << driver
                  << " is unknown, defaulting to OpenGL2\n";
        driverToUse = Window::Driver::OpenGL2;
        driver = "opengl2";
    }
    ...
    UTF8 windowName =
        "HPS Samples - " + testToRun->testName + UTF8(" - ") + driver;
