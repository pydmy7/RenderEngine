...
void Sample::Image_Background(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
     ...
     * to specify in which portfolio the image will end up in, instead of having to search
     * all portfolios.
     *
     * We will need to rewrite this sample to take advantage of that feature */

    /* The next block of code shows how to search portfolios for a specific image of which we know the name
     * 1. Obtain a list of all the portfolios in the database
     * 2. Check all portfolios for ImageDefinition objects
     * 3. When ImageDefinition objects are found, iterate through them checking whether their name matches the name
     *		of the desired image, using the ImageDefinition::Name() method */
    ...
    /* This block of code shows how to apply an ImageDefinition object to the window background
     * 1. The portfolio containing the image definition for the image to be applied to the window has to be visible at
     *		the window level. To make sure this is true, apply the portfolio to the Window's Style
     * 2. Set the image as the background by obtaining the SubwindowControl from a WindowKey, and using its
     *		SetBackground() method
     * 3. The first argument of SetBackground needs to be Subwindow::Background::Image to set the image as a background
     * 4. The second argument is the name of the image to be applied to the background */
    canvas.GetWindowKey().GetPortfolioControl().Push(pKey);
    canvas.GetWindowKey().GetSubwindowControl().SetBackground(Subwindow::Background::Image, image.Name());
