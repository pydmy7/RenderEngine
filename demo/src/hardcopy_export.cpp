...
void Sample::Hardcopy_Export(WindowKey wk, Canvas canvas, View view,
    ...
    /* This block of code shows you how to export a 2D-PDF and a Postscript file
     *representing the scene
     * 1. Use a Hardcopy::File::ExportOptionKit object to set the options needed
     *for the export
     * 2. To obtain the size of a window, get its WindowInfoControl object and
     *use its ShowPhysicalSize method
     * 3. Setting WYSIWYG on the export kit matches the model on screen to the
     *hardcopy output exactly
     * 4. The Hardcopy::File:Export method is where the format to export to is
     *specified
     * 5. Hardcopy::File::Export returns a IOResult object which can be checked
     *to see if the export operation completed successfully. */
    IOResult PDFResult = IOResult::Failure,
             postScriptResult = IOResult::Failure;
    ...
    try {
        ...
        exportOptionsKit.SetSize(width, height,
                                 Hardcopy::SizeUnits::Centimeters);
        exportOptionsKit.SetResolution(100, Hardcopy::ResolutionUnits::DPCM);

        PDFResult = Hardcopy::File::Export(
            pdfFileOut, Hardcopy::File::Driver::PDF, wk, exportOptionsKit);
        postScriptResult = Hardcopy::File::Export(
            psFileOut, Hardcopy::File::Driver::Postscript, wk,
            exportOptionsKit);
    } catch (IOException) {
        // handle Hardcopy exception
    }

    /*Here the Material Kit defined previously is attached to the description
     *subsegment Edges are set to visible The matMapKit is attached to the
     *description subsegment to ensure it's style does not affect the model Key
     *Material Mapping Style*/

    description.GetVisibilityControl().SetEdges(true);
    description.SetMaterialMapping(matMapKit);

    /* This block of code checks the result of the two export operations
     * completed above and prints out a message on the screen informing the user
