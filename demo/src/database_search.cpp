...
void Sample::Database_Search(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
     ...
     * - Obtain a SearchResultIterator object, and how to walk a search result list with it
     ...
     * 4. The third parameter of Find returns contains the results of the search, and is returned to the user
     * 5. The return value of Find indicates how many geometries matched our search criteria */
    SearchResults searchResults;
    UTF8 results;
    size_t numResults = modelKey.Find(Search::Type::Segment, Search::Space::SubsegmentsAndIncludes, searchResults);

    /* The next block shows you how to insert text
     * 1. Make sure that the visibility of text is turned on. You can explicitly turn it on
     *		with a call to GetVisibilityControl().SetText(true)
     * 2. Insert the text by specifying the position where you want the text to appear, and the text itself */
     ...
     * This time an iterator is obtained, so that the geometry matching the search criteria can be inspected
     * 1. Set up a while loop to walk though the SearchResult object with an iterator
     * 2. At the beginning of the loop check whether the iterator is valid with a call to its IsValid() method
     * 3. Obtain the key for the geometry the iterator is pointing to with a call to its GetItem() method
     * 4. You can complete different actions based on the type of the key returned by GetItem(). To find out
    ...
    numResults = modelKey.Find(Search::Type::Shell, Search::Space::SubsegmentsAndIncludes, searchResults);
#ifdef TARGET_OS_ANDROID
    sprintf(buf, "Number of segments: %u", numResults);
    results = buf;
#else
    ...
    results = buf;
#else
    results = "Number of vertices: " + UTF8(std::to_string(vertexCount).c_str());
#endif
