...
void Sample::Textures(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
    ...
    try {
        ...
        imageKit = Image::File::Import(static_cast<char const*>(file_in), importOptionsKit);
    }
    catch (IOException e) {
        UTF8 error_message;
     ...
     * An iterator is obtained, so that the geometry matching the search criteria can be inspected
     * 1. Use the Find() method to decide where to search and what to search for
     * 2. Obtain a SearchResultIterator object from the SearchResult returned to the user in Find()
     * 3. Set up a while loop to walk though the SearchResult object with an iterator
     * 4. At the beginning of the loop check whether the iterator is valid with a call to its IsValid() method
     * 5. Obtain the key for the geometry the iterator is pointing to with a call to its GetItem() method
     * 6. You can complete different actions based on the type of the key returned by GetItem(). To find out
     *		what kind of key was returned, compare the key type with one of those included in the HPS::Type enum
     * 7. Increment the iterator with a call to its Next() method*/
    SearchResults searchResults;
    modelKey.Find(Search::Type::Geometry, Search::Space::SubsegmentsAndIncludes, searchResults);
    ...
    /* The next block of code shows how to define a texture and how to apply it to geometry
     * 1. Obtain the key of a portfolio. The portfolio needs to be visible from the segment which will be using it
     *		therefore the portfolio obtained here is the one set on the Model
     * 2. Define an image, by giving using the PortfolioKey::DefineImage()
     *		the first argument of DefineImage is the name to be given to the image
     *		the second argument of DefineImage is the imageKit associated with the image from which the texture
     *		will be created.
     * 3. Choose a parameterization source for the texture which will be created using the PortfolioKey::SetParameterizationSource
     * 4. Define a texture using the PortfolioKey::DefineTexture()
     *		the first argument of DefineTexture is the name to be given to the texture
     *		the second argument of DefineTexture is the ImageDefinition object returned by the call to DefineImage
     *		the third argument of DefineTexture is a TextureOptionsKit containing texture options such as parameterization source
     * 5. To apply a texture to the geometry contained in a segment, obtain its MaterialMapping control and use the
     *SetFaceTexture() method*/
    ...
    textureOptionsKit.SetParameterizationSource(Material::Texture::Parameterization::Sphere);
    pKey.DefineTexture("wood_sphere", imageDefinition, textureOptionsKit);

    // Defining cylinder texture
    textureOptionsKit.SetParameterizationSource(Material::Texture::Parameterization::Cylinder);
    pKey.DefineTexture("wood_cylinder", imageDefinition, textureOptionsKit);

    // Defining UV texture
    textureOptionsKit.SetParameterizationSource(Material::Texture::Parameterization::UV);
