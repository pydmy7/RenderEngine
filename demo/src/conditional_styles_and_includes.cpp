...
void Sample::Conditional_Styles_And_Includes(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
     ...
     * 3. Since light does not transmit through windows, each subwindow needs a light source
     * 4. The second model is scaled down so that its size is comparable to the first model */
    model2.GetModellingMatrixControl().Scale(0.01f, 0.01f, 0.01f).Translate(0, 0.03f, 0);
     ...
     * 1. For each style create a segment which contains the attribute for the style
     * 2. Obtain the key for a portfolio
     * 3. Define named styles by giving them a name and the segment key that contains the
     *		attributes for each style */
    SegmentKey style1, style2, style3, style4;
    style1 = modelKey.Subsegment();
    style1.GetVisibilityControl().SetFaces(false).SetEdges(true).SetLights(false);
    style2 = modelKey.Subsegment();
    style2.GetLightingAttributeControl().SetInterpolationAlgorithm(Lighting::InterpolationAlgorithm::Phong);
     ...
     * The segment model1 will only use the 'wireframe' style is the condition 'bottom'
     * evaluates to TRUE
     *
     * If more than one condition evaluates to true, than multiple styles are used */
    ...
    /* The next block of code sets multiple conditions on each of the four subwindows
     * and includes the geometry which has the conditional styles set on it */
