...
void Sample::Hidden_Line_Advanced(WindowKey wk, Canvas canvas, View view,
    ...
    /* The next block of code shows how to change world handedness and how to
     * change culling settings
     * 1. World Handedness is a drawing attribute. From a SegmentKey object,
     * obtain its DrawingAttributeControl and use it to set the world handedness
     * 2. In order to change culling options, obtain a CullingControl object
     * 3. Using a CullingControl object, change the extent, back-face, frustum
     * and vector culling as desired*/
    view.GetSegmentKey().GetDrawingAttributeControl().SetWorldHandedness(
     ...
     * 3. Additional shadows attribute that can be set include the position of
     * the plane on which shadows will be rendered, the direction of the light
     * used for shadow generation, and the color of the shadows
     * 4. Enable shadow visibility through a VisibilityControl or a
     * VisibilityKit */
    view.GetSegmentKey()
     ...
     * 4. Choose the algorithm used for transparency calculations. In this case
     * Depth Peeling is selected
     * 5. Some transparency algorithm have special options - for example when
     * choosing Depth Peeling, the number of depth peeling layers can be
     * selected. In this example 2 layers are used
     * 6. Add an alpha component to the material color of the scene elements
     * which will be displayed as transparent. In this example, faces will be
     * 80% transparent while edges will be completely opaque. */
    view.GetSegmentKey()
    ...
    modelKey.GetMaterialMappingControl()
        .SetFaceColor(RGBAColor(1, 1, 1, 0.2f))
        .SetEdgeColor(RGBAColor(0, 0, 1, 1));

    /* The next block of code shows how to use the FastHiddenLine rendering
     * algorithm
     * 1. Obtain the SubwindowControl object, and change the rendering algorithm
     * with a call to SetRenderingAlgorithm
     * 2. To modify hidden line attributes, obtain the
     * HiddenLineAttributeControl object
     * 3. In this example faces are rendered so that shadow calculation can take
     * place even while in hidden line*/
    modelKey.GetVisibilityControl().SetFaces(true).SetEdges(true).SetLines(
        true);
    modelKey.GetSubwindowControl().SetRenderingAlgorithm(
        Subwindow::RenderingAlgorithm::FastHiddenLine);
    modelKey.GetHiddenLineAttributeControl().SetRenderFaces(true).SetVisibility(
        true);
}
