...
void Sample::Highlighting(WindowKey wk, Canvas canvas, View view, Model model, SegmentKey modelKey)
{
    ...
    try {
        ...
        Stream::ImportNotifier notifier = Stream::File::Import(static_cast<char const*>(filename), importOptionsKit);
        notifier.Wait();

        /* This block of code shows you how to load an image from a file
         * 1. Create an ImportOptionsKit and set the image format
         * 2. Use the Image::File::Import method to load the desired file with the options you specified
    ...
    // create an ImageDefinition from the image loaded earlier. an image definition
    // is required to create a TextureDefinition
    ...
    modelKey.GetModellingMatrixControl().Rotate(-90, 30, 0).Scale(0.6f, 0.6f, 0.6f);

    // create the opaque red style
    SegmentKey redStyleSegment = Database::CreateRootSegment();
    NamedStyleDefinition red_style = myPortfolio.DefineNamedStyle("red_style", redStyleSegment);
    redStyleSegment.GetMaterialMappingControl().SetFaceColor(RGBAColor(1.0f, 0, 0));
    ...
    NamedStyleDefinition transparent_style = myPortfolio.DefineNamedStyle("transparent_style", transparentStyleSegment);
    transparentStyleSegment.GetMaterialMappingControl().SetFaceColor(RGBAColor(1.0f, 0, 0, 0.5f));

    // create the textured style based on the image we loaded earlier
    SegmentKey texturedStyleKey = Database::CreateRootSegment();
    NamedStyleDefinition textured_style = myPortfolio.DefineNamedStyle("textured_style", texturedStyleKey);
    textured_style.GetSource().GetMaterialMappingControl().SetFaceTexture("my_texture");
    ...
    TextKit textKit;
    textKit.SetText("Unstyled model").SetPosition(Point(0, 0, -1.5f)).SetAlignment(HPS::Text::Alignment::Center);
    one_legend.InsertText(textKit);
    one_legend.GetDrawingAttributeControl().SetDepthRange(0, 0.1f); // bring to front
    ...
    two_legend.GetDrawingAttributeControl().SetDepthRange(0, 0.1f); // bring to front

    // Obtain a key path to the second subwindow, and highlight it using the red style setup earlier
    SprocketPath sprocket_path(canvas, canvas.GetAttachedLayout(), view, model);
    KeyPath key_path = sprocket_path.GetKeyPath().PushFront(two_include).PushFront(data_segment);
    ...
    three_legend.GetDrawingAttributeControl().SetDepthRange(0, 0.1f); // bring to front

    key_path = sprocket_path.GetKeyPath().PushFront(three_include).PushFront(data_segment);
    ...
    four_legend.GetDrawingAttributeControl().SetDepthRange(0, 0.1f); // bring to front
    key_path = sprocket_path.GetKeyPath().PushFront(four_include).PushFront(data_segment);
