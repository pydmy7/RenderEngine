...
namespace std {
    ...
    struct hash<HPS::RGBColor> {
        ...
        {
            return ((51 + std::hash<float>()(x.red)) * 51 + std::hash<float>()(x.green) * 51 + std::hash<float>()(x.blue));
        }
    };
} // namespace std
...
namespace HPS {
    ...
    class SPRK_OPS_API PanOrbitZoomOperator: public Operator {
      public:
        PanOrbitZoomOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
         ...
         *  This function interprets the input to decide whether to orbit, zoom or pan the camera, and updates the camera as
         * needed. \param in_state A MouseState object describing the current mouse state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
         ...
         *  This function interprets the input to decide whether to orbit, zoom or pan the camera, and updates the camera as
         * needed. \param in_state A TouchState object describing the current touch state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchMove(TouchState const& in_state);

      private:
        HPS::WorldPoint operator_movement_world_space_start;
        ...
        std::shared_ptr<HPS::Point const> selection_target;

        struct StartingMovementCameraState {
            HPS::Point const position;
            HPS::Point const target;
            HPS::Vector const look;
            float const focal_length;
            float const field_width;
            float const field_height;
            HPS::MatrixKit const projection_to_world;
        };

        std::shared_ptr<StartingMovementCameraState const> starting_movement_camera_state;

        size_t current_touches;

        void SetStartingMovementCameraState(HPS::KeyPath const& key_path);
        void UpdateZoom(float zoom_factor);
        void UpdateZoomLimit();
        void UpdatePan(HPS::WorldPoint const& newLocation);

        void UpdateZoomPan(float zoom_factor, HPS::WorldPoint const& new_location_pan);

        /*! Update zoom and core of the camera.
         * \param zoom_factor Applied Zoom factor. If nullptr, no zoom is applied at all.
         * \param new_location_pan Translated position of the camera after a pan movement. If nullptr, no offset is applied.
         *   */
        void UpdateZoomPanCore(std::unique_ptr<float const> const& zoom_factor,
                               std::unique_ptr<HPS::WorldPoint const> const& new_location_pan);
    };
    ...
    class SPRK_OPS_API PanOperator: public Operator {
      public:
        PanOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                    ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API OrbitOperator: public Operator {
      public:
        OrbitOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                      ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API RelativeOrbitOperator: public Operator {
      public:
        ...
        */
        RelativeOrbitOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonMiddle(),
                              ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API ZoomOperator: public Operator {
      public:
        ZoomOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                     ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API ConstructRectangleOperator: public Operator {
      public:
        ConstructRectangleOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                                   ModifierKeys in_modifier_trigger = ModifierKeys(),
    ...
    class SPRK_OPS_API ZoomBoxOperator: public ConstructRectangleOperator {
      public:
        ZoomBoxOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                        ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    */
    class SPRK_OPS_API SelectAreaOperator: public ConstructRectangleOperator {
      public:
        ...
        */
        SelectAreaOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                           ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API HighlightAreaOperator: public SelectAreaOperator {
      public:
        ...
        */
        HighlightAreaOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                              ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    */
    class SPRK_OPS_API SelectOperator: public Operator {
      public:
        /*!
            ...
            \param in_modifier_trigger The modifier key that, when used with the
                                       ...
                                       modifier key is necessary to trigger the operator.
        */
        SelectOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                       ModifierKeys in_modifier_trigger = ModifierKeys());
      ...
      private:
        bool SelectCommon(HPS::Point const& in_loc,
                          HPS::WindowKey& in_window,
                          HPS::ModifierKeys in_modifiers = HPS::ModifierKeys());
    ...
    class SPRK_OPS_API HighlightOperator: public SelectOperator {
      public:
         ...
         */
        HighlightOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                          ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API MouseWheelOperator: public Operator {
      public:
        ...
        * \param in_alternate_type_modifiers The modifier button that, when pressed, will activate the zoom type opposite to that
        specified in in_default_type.
            Passing an empty ModifierKey object prevents the zoom type from being anything but what was specified in
        in_default_type. */
        MouseWheelOperator(ZoomType in_default_type = ZoomType::Accurate,
    ...
    class SPRK_OPS_API TurntableOperator: public Operator {
      public:
        TurntableOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                          ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    class SPRK_OPS_API FlyOperator: public Operator {
      public:
        FlyOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonRight(),
                    ModifierKeys in_modifier_trigger = ModifierKeys());
        ...
        /* Keyboard sensitivity affects the speed of movement of action triggered from the keyboard, such as walking forward by
         * pressing W */
        ...
        {
            left_joystick_sensitivity = in_left_joystick_sensitivity;
        }
        ...
        {
            right_joystick_sensitivity = in_right_joystick_sensitivity;
        }
      ...
      private:
        enum class TouchPosition { Left, Right, None };
    ...
    while snapped to a plane. This operator is intended for walking through models that have a defined ground plane, such as a
    floor in a building. For models that don't have a ground plane, Visualize will attempt to determine a ground plane
    heuristically.
     *  This operator works for both mouse and touch-driven devices.
     * \htmlonly
     ...
     *    <tr><td class="td-bk customDataTable_leftColumn">W:</td><td class="td-bk">Move camera position and target
    forward</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">S:</td><td class="td-bk">Move camera position and target
    backward</td></tr>
     ...
     *    <tr><td class="td-bk customDataTable_leftColumn">Right-button down and move:</td><td class="td-bk">Rotate camera in
    place</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Shift:</td><td class="td-bk">Walk at double the speed while
    pressed</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Ctrl:</td><td class="td-bk">Walk at half the speed while
    pressed</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Mouse wheel:</td><td class="td-bk">Increase / decrease step
    length</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Shift + mouse wheel:</td><td class="td-bk">Increase / decrease field of
    view</td></tr>
     * </table>
     *
     *
     * <h3>Touch devices:</h3>
     * <table>
     *    <tr><td class="td-bk customDataTable_leftColumn">Touch left side of screen:</td><td class="td-bk">Pans camera in left or
    right, moves camera forward or back.</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Touch right side of screen:</td><td class="td-bk">Rotates camera in
    place.</td></tr>
     *    <tr><td class="td-bk customDataTable_leftColumn">Two-finger touch and move one finger:</td><td class="td-bk">Pans camera
    up or down.</td></tr>
     *  <tr><td class="td-bk customDataTable_leftColumn">Double-touch anywhere on the screen:</td><td class="td-bk">Re-center the
    model.</td></tr>
     * </table>
     * <br/>
    ...
    class SPRK_OPS_API WalkOperator: public FlyOperator {
      public:
        WalkOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonRight(),
                     ModifierKeys in_modifier_trigger = ModifierKeys());
    ...
    *  <table>
    *    <tr><td class="td-bk customDataTable_leftColumn">Left-button down and move:</td><td class="td-bk">Dollies the camera
    forward or backward, or Pans the camera left or right based on the mouse movement direction. The movement velocity changes
    based on how far the mouse is dragged from the starting point.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Middle-button down and move:</td><td class="td-bk">Dollies the camera
    vertically or horizontally based on the mouse movement direction. The movement velocity changes based on how far the mouse is
    dragged from the starting point.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Shift:</td><td class="td-bk">With the Shift key down the
    forward/backward velocity is ten times faster.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Ctrl:</td><td class="td-bk">With the Ctrl key down the forward/backward
    velocity is ten times slower, and the left/right velocity is 45% slower. </td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Mouse wheel:</td><td class="td-bk">Pans the camera vertically. This will
    change the tilt direction that the camera is looking, but does not change the direction of forward/backward
    movement.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Shift + mouse wheel:</td><td class="td-bk">Increases or decreases the
    base velocity of Dolly operations.</td></tr>
    * </table>
    *
    *  <h3>Touch Operation:</h3>
    *  <table>
    *    <tr><td class="td-bk customDataTable_leftColumn">Single Finger Drag</td><td class="td-bk">Dollies the camera forward or
    backward, or Pans the camera left or right based on the touch movement direction. The movement velocity changes based on how
    far the touch is dragged from the starting point.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Two Finger Drag</td><td class="td-bk">Dollies the camera vertically or
    horizontally based on the touch movement direction. The movement velocity changes based on how far the touch is dragged from
    the starting point.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Shift:</td><td class="td-bk">With the Shift key down the
    forward/backward velocity is ten times faster.</td></tr>
    *    <tr><td class="td-bk customDataTable_leftColumn">Ctrl:</td><td class="td-bk">With the Ctrl key down the forward/backward
    velocity is ten times slower, and the left/right velocity is 45% slower. </td></tr>
    ...
    *  */
    class SPRK_OPS_API SimpleWalkOperator: public WalkOperator {
      public:
        SimpleWalkOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                           ModifierKeys in_modifier_trigger = ModifierKeys());
         ...
         * The <span class='code'>SimpleWalkOperator</span> has Camera Dolly operations fixed to the middle mouse button by
         * default. To disable this behavior, set this value to  <span class='code'>false</span>.
         ...
         * The <span class='code'>SimpleWalkOperator</span> has Camera Dolly operations fixed to the middle mouse button by
         * default. To disable this behavior, set this value to  <span class='code'>false</span>. \return <span
         * class='code'>true</span> if middle mouse button Camera Dollying is enabled, <span class='code'>false</span> otherwise.
         */
        bool GetEnableMiddleMouseButtonDolly() const;

        /*! This function overrides the base class implementation to additionally inspect <span class='code'>in_state</span> for
         * the middle mouse button state if middle mouse button Camera Dollying is enabled. \return <span class=code>true</span>
         * if the MouseButtons and ModifierKeys are active. */
      ...
      private:
        class ScaleFactor {
          public:
            ...
            ScaleFactor(float time): time(time), forward(1), horizontal(1), vertical(1), roll(1), pitch(1), yaw(1) {}
        ...
        bool Move(
            HPS::Point const& start, HPS::Point const& current, int directions, HPS::CameraKit& camera, ScaleFactor const& scale);
    ...
    /*!
     \htmlonly
     <p>The CuttingSectionOperator class defines an operator which allows the user to insert and interact with cutting sections.
     This Operator works for both mouse- and touch-driven devices. Each instance of the operator supports inserting and
     interacting with a single cutting plane. Multiple instances of the operator can be pushed to the View's operator stack when
     the user wants to add an additional cutting plane to the scene. The cutting plane associated with an instance of the operator
     will be removed when the operator is popped from the View's operator stack.</p>
     ...
     <table>
        ...
        <tr>
            ...
            <td class="td-bk">When the operator is first activated, moving the mouse over the model will display a indicator
                representing the normal of the face the user is currently hovering over. Once a cutting section has been inserted,
     it will be highlighted when the user hovers over it. The highlight options can also be customized, or this behavior can be
     disabled entirely.</td>
        </tr>

        <tr>
            <td class="td-bk" style="width: 33%;">Left click</td>
            <td class="td-bk">When the operator is first activated, a left click will insert a cutting section at this
     position.</td>
        </tr>
    ...
    class SPRK_OPS_API CuttingSectionOperator: public SelectOperator {
      ...
      public:
        CuttingSectionOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                               ModifierKeys in_modifier_trigger = ModifierKeys());
         ...
         *  After a cutting section has been inserted this function allows the user to translate a cutting section along its
         * normal \param in_state A MouseState object describing the current mouse state. \return <span class='code'>true</span>
         * if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnMouseMove(MouseState const& in_state);
        ...
        /*! This function is called whenever HPS receives a TouchEvent that signals a point of contact has moved.
         *  After a cutting section has been inserted this function allows the user to translate a cutting section along its
         * normal \param in_state A TouchState object describing the current touch state. \return <span class='code'>true</span>
         * if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchMove(TouchState const& in_state);
        ...
        /*! Returns the scale used for the normal indicator displayed while mousing over the model when the operator is first
         * started. \return The scale of the normal indicator. */
         ...
         * \param in_highlight_options The highlight options kit used when hovering over geometry, when mouse-over highlighting is
         * enabled. */
        void SetPlaneHighlightOptions(HighlightOptionsKit const& in_highlight_options)
        {
            highlight_options = in_highlight_options;
        }

        /*! Whether mouse-over highlighting is enabled
         * \return <span class='code'>true</span> if mouse-over highlighting is enabled, <span class='code'>false</span>
         * otherwise. */
        ...
        {
            enable_mouse_over_highlighting = in_enable_mouse_over_highlighting;
        }
         ...
         * \return <span class='code'>true</span> if the plane normal indicator is visible, <span class='code'>false</span>
         * otherwise. */
        bool GetIndicatorVisibility()
        {
            return (op_state == OpState::Uninitialized || op_state == OpState::FacePicking) ? true : false;
        }
         ...
         * \return <span class='code'>true</span> if the cutting sections plane representations are visible, <span
         * class='code'>false</span> otherwise. */
        bool GetPlaneVisibility();

        /* The operator needs to know the model bounding in order to display cutting sections of the appropriate size.
         * The bounding is calculated automatically the first time a section is inserted, and when the operator detects that a
         * Component has been Shown, Hidden, Isolated or its Visibility has been Reset. If changes are made to the scene outside
         * of the ones listed above, which should warrant the model bounding to be recalculated for
         * purposes of displaying cutting sections, the operator can be asked to do so by calling this function. */
        void RecalculateModelBounding();

      private:
        class ShowHideIsolateResetHandler: public HPS::EventHandler {
          public:
            ShowHideIsolateResetHandler(): EventHandler() {}

            virtual ~ShowHideIsolateResetHandler() { Shutdown(); }

            virtual HandleResult Handle(Event const* /*in_event*/)
            {
                op->model_bounding.Invalidate();
                if (!op->sections.empty())
                    op->ResizeCuttingPlanes();
                return HandleResult::Handled;
            }

            void SetOperator(CuttingSectionOperator* in_operator) { op = in_operator; }

          private:
            CuttingSectionOperator* op;
        };
        ...
        bool HandleMouseAndTouchDown(WindowKey const& in_event_source,
        ...
        void ResizeCuttingPlanes();

        struct SectionInfo {
            SectionInfo() {}

            SectionInfo(CuttingSectionKey const& in_cutting_section, ShellKeyArray const& in_shell_keys):
                cutting_section_key(in_cutting_section), shell_key_array(in_shell_keys)
            {
            }

            CuttingSectionKey cutting_section_key;
            ShellKeyArray shell_key_array;
        };
        ...
        ShowHideIsolateResetHandler handler;
    };
    ...
    /*!
     ...
     - Rectangle:
        ...
        For touch driven devices, there are two options available for inserting rectangle markups:
            a. tap where you want to place the top left corner of the rectangle and move your finger to change the position of the
     bottom right corner. b. tap with two fingers. Move and pinch with the fingers to change the position and size of the
     rectangle.
    ...
    class SPRK_OPS_API MarkupOperator: public Operator {
      public:
        ...
        MarkupOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                       ModifierKeys in_modifier_trigger = ModifierKeys());
        ...
        /*! This function is called whenever HPS receives a MouseEvent that signals a mouse button was pressed.
         *  This function starts inserting markup. In the case of text markup, this function determines where the text will be
         * inserted. \param in_state A MouseState object describing the current mouse state. \return <span
         * class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        ...
        /*! This function is called whenever HPS receives a TouchEvent that signals the device was touched.
         *  This function starts inserting markup. In the case of text markup, this function determines where the text will be
         * inserted. \param in_state A TouchState object describing the current touch state. \return <span
         * class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        ...
        {
            current_attributes.text_attributes = in_text_attributes;
        }
        ...
        {
            current_attributes.line_attributes = in_line_attributes;
        }
        ...
        class SPRK_OPS_API MarkupInsertedEvent: public HPS::Event {
          public:
            ...
            char const* ClassName() const override { return "MarkupInsertedEvent"; }
      ...
      private:
        ...
        class KeyboardHiddenEventHandler: public EventHandler {
          public:
            KeyboardHiddenEventHandler(): EventHandler() {}

            virtual ~KeyboardHiddenEventHandler() { Shutdown(); }
        ...
        static RGBColor const default_color;
        static TextAttributeKit const default_text_attributes;
        static LineAttributeKit const default_line_attributes;
    ...
    class SPRK_OPS_API AnnotationOperator: public Operator {
      public:
        AnnotationOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                           ModifierKeys in_modifier_trigger = ModifierKeys());
        ...
        /*! This function is called whenever HPS receives a TouchEvent that signals a point of contact has moved.
         *  When users are inserting or editing an annotation, they will be able to reposition it by tapping it and dragging their
         * finger \param in_state A TouchState object describing the current touch state. \return <span class='code'>true</span>
         * if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchMove(TouchState const& in_state) override;
        ...
        class SPRK_OPS_API AnnotationInsertedEvent: public HPS::Event {
          public:
            ...
            char const* ClassName() const override { return "AnnotationInsertedEvent"; }
      ...
      private:
        ...
        KeyboardHiddenEventHandler
            handler; // a handler that takes care of resetting the camera when the software keyboard is hidden
        Point touch_down_position; // the position the last touch down occurred at
        bool always_show_keyboard_on_touch_up; // true if we are just inserting the annotation, false if we are editing a
                                               // previously inserted one.
    };

    /*!
     * The HandlesOperator class defines an operator which allows the user to translate geometry in the view plane or along an
     * axis. This operator works for both mouse- and touch-driven devices.
     *
     ...
     *
     * This operator does not work on Exchange models which have a Drawing structure. This includes CATDrawing models and 2D DWG
     * models loaded through Exchange. Any attempt to use this operator on such a model will produce a warning.
     */
    class SPRK_OPS_API HandlesOperator: public Operator {
      public:
        HandlesOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                        ModifierKeys in_modifier_trigger = ModifierKeys());
        ~HandlesOperator();
         ...
         * \param in_handles_points The number of points used to draw the handles. A higher number will yield a smoother
         * appearance. Valid range is > 0. */
         ...
         * Changes will take effect the next time handles are inserted.
         * \param in_offset The distance between the end of a translation handle and the start of then rotation handle associated
         * with it, specified as a percentage of handle length. Valid range is (0, 1]. \param in_angle The angle formed by the
         * rotation handles, specified in degrees. Valid range is > 0 \param in_tip_size The size of the tip at the end of the
         * handles, specified as a percentage of the handle length. Valid range is [0, 1]. */
         ...
         * Changes will take effect the next time handles are inserted.
         * \param in_plane_offset The distance between the axis translation handle and the start of the plane translation handle,
         * specified as a percentage of handle length. \param in_plane_length The size of plane translation handles, specified as
         * a percentage of handle length.
         * \param in_center_radius The radius of the sphere at the center of the handles, specified as a percentage of handle
         * length. */
        void SetPlaneTranslationAppearance(float in_plane_offset = 0.2f,
         ...
         * \param out_handles_points The number of points used to draw the handles. A higher number will yield a smoother
         * appearance. */
         ...
         * \param out_offset The distance between the end of a translation handle and the rotation handle associated with it,
         * specified as a percentage of handle length. \param out_angle The angle formed by the rotation handles, specified in
         * degrees.
        ...
        /*! Returns the parameters used to decide the appearance of the rotation handles.
         * \param out_plane_offset The distance between the axis translation handle and the start of the plane translation handle,
         * specified as a percentage of handle length. \param out_plane_length The size of plane translation handles, specified as
         * a percentage of handle length. \param out_center_radius The radius of the sphere at the center of the handles,
         * specified as a percentage of handle length. */
         ...
         * Trailing geometry is geometry which shows you the path of the last movement operation you performed through the
         * handles. \param in_state Whether to draw trailing geometry when using the handles. */
         ...
         * Trailing geometry is geometry which shows you the path of the last movement operation you performed through the
         * handles. \return <span class='code'>true</span> if trailing geometry will be drawn, <span class='code'>false</span>
         * otherwise. */
        ...
        class SPRK_OPS_API GeometryTransformedEvent: public HPS::Event {
          public:
            ...
            GeometryTransformedEvent(HPS::View const& in_view,
                                     HPS::SegmentKey const& in_segment_key,
                                     HPS::MatrixKit const& in_transform):
                Event()
            {
                channel = GetClassID();
                consumable = false;
            ...
            char const* ClassName() const override { return "GeometryTransformedEvent"; }
      ...
      private:
        ...
        CanvasArray canvases; // A list of Canvas objects that the View attached to this operator belongs to. This value only gets
                              // updated on OnViewAttached.
        ...
        float arc_offset; // Distance between the end of the translation handle and the start of the translation arc. Expressed in
                          // percentage of handles_length [0 - 1].
        ...
        float plane_offset; // The distance between the axis translation handle and the start of the plane translation handle,
                            // specified as a percentage of handles_length.
        ...
        bool
            InputDown(size_t in_click_count, WindowKey const& in_window, KeyArray const& in_event_path, Point const& in_location);
    ...
    /*!
     * The MeasurementOperator class defines an operator which allows the user to measure geometry.
     * This operator responds to both mouse and touch events.
     * Five different measuring types are available: Point to Point, Line, Angle, Area and Distance.
     * Whenever a measurement is inserted, an Event of type MeasurementEvent is injected. This event can be handled in order to
     * to get details about the measurement (its type and value), which View it was inserted under, and what Segment it was placed
     * on. This information, and specifically the segment in which the measurement is placed, can be used to customize the
     * appearance of the measurement, to delete it, hide it, etc...
     *
     * When vertex snapping is enabled, the measurement operator will try to snap to vertices close to the location that was
     * clicked / touched. The value of the proximity snapping is expressed in centimeter. Snapping is not taken into consideration
     * when placing Distance measurements.
     *
     * If hinting is turned on, which it is by default, the operator will give the user feedback on what would be measured if a
     * click were to happen at the current position. This feedback is given in the form of markers of various shapes. By default,
     * an X means that the clicking the current point will not yield a measurement point. This can happen if the click happens off
     * the geometry or if the current point would otherwise be invalid (for example, if the point would not be on the same plane
     * as the other points which describe an area measurement). When vertex snapping is enabled, a square denotes the position the
     * measurement point would snap to. Finally, a circle indicates that the current selection is valid but would not be snapped
     * to a vertex. The look of these hinting geometry can be changed at anytime by editing the attributes found in their owning
     * segments. The segments can be obtained through a call to GetHintSegment Hinting geometry is not enabled for Distance
     * measurement, since the individual points picked are not relevant in this mode.
     *
     * The number of decimal place shown in measurement labels can be changed through the SetPrecision function, and defaults
     * to 2.
     *
     * When working with a CADModel, the CADModel can be passed to this operator through the SetCADModel function.
     * Setting a CADModel allows the operator to detect the unit of measurements used in the loaded file, which will be displayed
     * when inserting measurement labels. When a CADModel is not set, measurements are considered unit-less.
     *
     * While inserting a measurement, the last point inserted can be undone by pressing CTRL+Z. Note that in order to delete a
     * single measurement that was already inserted, you should delete its associate segment, which can be obtained by
     * intercepting MeasurementEvent events. Deleting all measurements at once can be easily done through the
     * DeleteAllMeasurements function.
     *
     * Notes about specific measurement types:
     * - Line Measurements
     * Lines are measured as points are picked. An arbitrary number of points describing a line can be picked.
     * To signal that you are done describing a line, either select the last point inserted, or double click/tap anywhere in the
     * scene The position of the label for Line Measurement can be chosen through the SetLineMeasurementPosition.
     *
     * - Area Measurements
     * Placing at least three points is required when measuring an area. All points placed should be co-planar. After the third
     * point has been inserted, attempting to insert a point that does not lie on the same plane will result in a warning, and the
     * operator will discard this point. To signal that you are done inserting points for the area measurement, either select the
     * first point inserted, or double click/tap anywhere in the scene.
     *
     * - Distance Measurements
     * The distance measurement finds the shortest distance between two objects.
     * The two objects in question can be either individual faceted Entities (shells, polygons, meshes, etc...) or Bodies.
     * This setting can be controlled through the SetDistanceMeasurementReference function.
     * When Bodies are selected as the reference, is a CADModel is not associated with this operator, Segments will be used
     * instead. Note that when comparing whole bodies it might take a noticeable amount of time for the operator to complete the
     * calculation.
     */
    class SPRK_OPS_API MeasurementOperator: public Operator {
      public:
        MeasurementOperator(MouseButtons in_mouse_trigger = MouseButtons::ButtonLeft(),
                            ModifierKeys in_modifier_trigger = ModifierKeys());
        ~MeasurementOperator();
        ...
        enum class LineMeasurementPosition {
        ...
        enum class DistanceMeasurementReference {
...
#if TARGET_OS_IPHONE == 0 && TARGET_OS_ANDROID == 0
        enum class SelectionHint{
            Snapped, //!< If vertex snapping is enabled, hint goemetry is rendered over the closest vertex within the snapping
                     //!< proximity supplied to SetVertexSnapping.
            Valid, //!< Hint goemetry is rendered under the mouse cursor indicating that a valid measurement point could be
                   //!< inserted at the underlying position. If vertex snapping is enabled this also indicates that the current
                   //!< position is outside the proximity specified to SetVertexSnapping.
            Invalid, //!< Hint goemetry is rendered under the mouse cursor indicating that no valid measurement point could be
                     //!< inserted at the underlying position. This could be due to the cursor being over no visible geometry or
                     //!< the underlying position being invalid for usage with the current measurement type.
        };

        /*! Retrieves the segment key which contains the marker used for a given snapping hint.
        * The appearance of the marker used for each hint type can be altered by modifying the attributes of the returned segment
        key.
        * \param in_hint Indicates the hint type to retrieve the parent segment for.
        which contains the marker used to indicate the snapping state given the mouse position. */
        SegmentKey GetHintSegment(SelectionHint in_hint);
#endif

        /*! Returns the name of the operator. */
        virtual HPS::UTF8 GetName() const override { return "HPS_MeasurementOperator"; }
        ...
        /*! This function is called whenever HPS receives a MouseEvent that signals a mouse button was pressed.
         *  This function starts the operator and records the position of the mouse click
         * \param in_state A MouseState object describing the current mouse state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnMouseDown(MouseState const& in_state) override;
        /*! This function is called whenever HPS receives a MouseEvent that signals the mouse moved
         *  This function shows a preview of what would be selected, taking into account vertex snapping
         * \param in_state A MouseState object describing the current mouse state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnMouseMove(MouseState const& in_state) override;
        /*! This function is called whenever HPS receives a MouseEvent that signals a mouse button was released.
         *  If the mouse position was the same as it was when the mouse button was pressed, a measurement point is inserted.
         * \param in_state A MouseState object describing the current mouse state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnMouseUp(MouseState const& in_state) override;

        /*! This function is called whenever HPS receives a TouchEvent that signals that the device was touched.
         *  This is the touch equivalent of OnMouseDown
         * \param in_state A TouchState object describing the current touch state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchDown(TouchState const& in_state) override;
        /*! This function is called whenever HPS receives a TouchEvent that signals that a touch on the device has moved
         *  This is the touch equivalent of OnMouseMove
         * \param in_state A TouchState object describing the current touch state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchMove(TouchState const& in_state) override;
        /*! This function is called whenever HPS receives a TouchEvent that signals that a touch on the device was released
         *  This is the touch equivalent of OnMouseUp
         * \param in_state A TouchState object describing the current touch state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnTouchUp(TouchState const& in_state) override;

        /*! This function is called whenever HPS receives a KeyboardState event that signals a button was pressed.
         *  This function is used to undo a measurement point when CTRL+Z is pressed
         * \param in_state A KeyboardState object describing the current keyboard state.
         * \return <span class='code'>true</span> if the input event was handled, <span class='code'>false</span> otherwise. */
        virtual bool OnKeyDown(KeyboardState const& in_state) override;

        /*! Sets the type of measurement to insert.
         *	If a measurement of a different type is currently being inserted, that measurement will be deleted. */
        ...
        MeasurementType GetMeasurementType() const { return current_measurement_type; }

        /*! Whether selections should snap to the closest vertex.
         * This behavior is only relevant when selecting shells, meshes, lines, NURBS curves and polygons.
         * If the selection cannot be snapped to a vertex, the measurement will be taken based on the actual selection point.
         * \param in_state Indicates whether to enable or disable vertex snapping.
         * \param in_proximity The distance to which a vertex will snap. The value is specified in centimeters. */
...
#if TARGET_OS_IPHONE == 0 && TARGET_OS_ANDROID == 0
        /*!  Whether to show hint geometry while moving the mouse over the model during point selection.
         *	 This function is not available on mobile platforms as it requires hovering over geometry.
         ...
         * The operator might override this setting if increasing the precision is necessary in order not to display the
         * measurement value as all zeros. Example: When precision is set to 2, and a distance measurement of 0.0012 is performed,
         * the operator will display 3 digits after the decimal point (0.001) in order not to display a value of 0. Only affects
         * future measurements. */
        ...
        void SetLineMeasurementPosition(LineMeasurementPosition in_position) { line_measurement_position = in_position; }

        /*! Returns where the line measurement text will be positioned. */
        LineMeasurementPosition GetLineMeasurementPosition() const { return line_measurement_position; }

        /* Sets the reference used when performing distance measurements. The default is Body.
         *	When Body is selected, and an Exchange or Parasolid CADModel is associated with this operator,
         *	the minimum distance between two selected Representation Items (for Exchange) or Bodies (for Parasolid) will be
         *computed. If Entity is selected, the distance will be calculated between the topological faces being selected When a
         *CADModel is not associated with this operator, Body is a synonym for Segment. */
        void SetDistanceMeasurementReference(DistanceMeasurementReference in_reference)
        {
            distance_measurement_reference = in_reference;
        }

        /*! Returns the reference used for distance measurements. */
        DistanceMeasurementReference GetDistanceMeasurementReference() const { return distance_measurement_reference; }

        /* An event which will be injected when a measurement is inserted into the scene through this operator.
         * Intercepting this event through a class derived from HPS::EventHandler allows for fine control over measurements
         * by having access to their backing SegmentKey.
         * Information about the inserted measurement, such as its type and value is also conveyed through this Event */
        class SPRK_OPS_API MeasurementEvent: public HPS::Event {
          public:
            ...
            MeasurementEvent(HPS::View const& in_view,
                             HPS::SegmentKey const& in_segment_key,
                             MeasurementOperator::MeasurementType in_measurement_type,
                             float in_measurement_value):
                Event(),
                view(in_view), segment_key(in_segment_key), measurement_type(in_measurement_type),
                measurement_value(in_measurement_value)
            {
                channel = GetClassID();
                consumable = false;
            }
             ...
             * 	\param in_event The Event Object to be converted. */
            MeasurementEvent(Event const& in_event): Event(in_event)
            {
                if (in_event.GetChannel() == Object::ClassID<MeasurementEvent>()) {
                    auto that = static_cast<MeasurementEvent const&>(in_event);
                    view = that.view;
                    segment_key = that.segment_key;
                    measurement_type = that.measurement_type;
            ...
            char const* ClassName() const override { return "MeasurementEvent"; }

            /*! Allocates and returns a copy of this MeasurementEvent.
             * 	\return A copy of this MeasurementEvent. */
            Event* Clone() const
            {
                MeasurementEvent* new_event = new MeasurementEvent(*this);
                return new_event;
            }
      ...
      private:
...
#if TARGET_OS_IPHONE == 0 && TARGET_OS_ANDROID == 0
        void SetHintState(SelectionHint in_state, HPS::Point const& world_space_point);
#endif
        bool EstablishSelectionScope(HPS::KeyArray const& event_path);
        bool
            InputDown(size_t in_click_count, WindowKey const& in_window, KeyArray const& in_event_path, Point const& in_location);
        bool InputMove(WindowKey const& in_window, KeyPath const& in_path, Point const& in_location);
        void InputUp(WindowKey const& in_window, KeyArray const& in_event_path, Point const& in_location);

        SegmentKey CreateMeasurementSegment();

        void InsertAngleMeasurement();
        void InsertPointToPointMeasurement();
        void InsertAreaMeasurement(bool in_last_point = false);
        void InsertLineMeasurement(bool in_last_point = false);
        void InsertDistanceMeasurement();

        void RemoveMeasurementPoint();

        void CalculateAndInsertArea();
        UTF8 FormatMeasurementText(float measurement, HPS::UTF8 const& suffix_str = HPS::UTF8());
        TextKey InsertMeasurementText(Point const& text_insertion_point,
                                      float measurement,
                                      HPS::UTF8 const& suffix_str = HPS::UTF8());
        void FinalizeMeasurement(float measurement);
        void CleanupActiveMeasurement(bool delete_segment = false);
        Point CalculateLineMeasurementPosition();
        ...
        double
            ClosestPointSegmentSegment(Point const& p1, Point const& q1, Point const& p2, Point const& q2, Point& c1, Point& c2);

        bool SelectSnappingToVertex(WindowKey const& in_window, Point const& in_location, bool& point_is_snap_point);
        bool SelectSnappingToVertex(WindowKey const& in_window, Point const& in_location);
        bool SelectWithoutSnapping(WindowKey const& in_window, Point const& in_location);

        Point GetWorldPointFromVertex(size_t in_vertex_index, Key in_owning_geometry, KeyPath const& in_path_to_geometry);

        template<class T>
        Point GetPointFromGeometry(size_t in_point_index, T in_geometry)
        {
            PointArray points;
            if (in_geometry.ShowPointsByRange(in_point_index, 1, points))
                return points[0];
            return Point();
        }

        bool MeasurementLabelUnderCursor(WindowKey const& in_window, WindowPoint const& in_location, TextKey& out_text);
        static std::mutex index_mutex;
        static size_t point_to_point_index;
        static size_t line_index;
        static size_t angle_index;
        static size_t area_index;
        static size_t distance_index;

        TouchID tracked_touch_ID; // for touch events, the ID of the touch that started the measurement insertion
        MeasurementType current_measurement_type; // what kind of measurement is currently being inserted
        bool measurement_in_progress; // true is a  measurement is in the process of being inserted
        HighlightOptionsKit highlight_options;
        SegmentKey measurement_segment;
        View last_attached_view;
        CADModel cad_model;
        HPS::Type cad_model_type;
        UTF8 length_units;
        UTF8 area_units;
        WindowPoint input_down_location;
        size_t input_down_click_count;

        // Selection Members
        bool established_selection_scope;
        SelectionOptionsKit subentity_selection_options;
        SelectionOptionsKit entity_selection_options;
        SelectionOptionsKit markup_selection_options;
        TreeContext tree_context;

        // members only valid if a measurement is being repositioned
        bool measurement_selected;
        PointArray selected_measurement_points;
        Point selected_measurement_position;
        MeasurementType selected_measurement_type;
        TextKey selected_measurement_text;
        LineKeyArray selected_measurement_leaders;
        SegmentKeyArray selected_measurement_segments;
        Vector selected_camera_direction;
        CircularArcKey selected_angle_arc;
        Plane selected_angle_plane;

        // Construction Geometry Members
        SegmentKey current_measurement_segment; // segment where the current measurement should be inserted

        // Angle specific members
        CircularArcKey angle_arc;
        TextKey angle_text;
        float last_angle_measurement;
        bool angle_measurement_inserted;

        // selection hint members
#if TARGET_OS_IPHONE == 0 && TARGET_OS_ANDROID == 0
        bool selection_hint_enabled;
        bool selection_hint_active;
        SelectionHint selection_hint_state;
        SegmentKey preview_marker_root_segment;
        SegmentKey snapped_marker_segment;
        MarkerKey snapped_marker;
        SegmentKey valid_marker_segment;
        MarkerKey valid_marker;
        SegmentKey invalid_marker_segment;
        MarkerKey invalid_marker;
#endif

        // snapping data
        bool snap_to_vertex;
        float snap_proximity;
        size_t measurement_precision;

        // shared measurement variables
        PointArray measurement_points;
        bool processing_double_click;

        // Area specific data
        Vector area_normal;

        // Line measurement variables
        LineMeasurementPosition line_measurement_position;
        LineKey last_inserted_line;
        TextKey last_inserted_length;
        float line_length;

        // distance measurement variables
        KeyPathArray selected_objects;
        TypeArray selected_objects_types;
        HighlightOptionsKit distance_measurement_highlight;
        SegmentKey distance_measurement_style_segment;
        HighlightOptionsKit distance_measurement_line_highlight;
        SegmentKey distance_measurement_line_style_segment;
        SegmentKey scratch_segment;
        DistanceMeasurementReference distance_measurement_reference;
    };
